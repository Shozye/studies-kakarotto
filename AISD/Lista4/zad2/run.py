...
pbar = tqdm(product(INSERT_TYPES, range(START, END, STEP)), "Parsing...")
for insert_type, n in pbar:
    pbar.set_description(f"Parsing {insert_type} for amount {n}")
    data[insert_type][n] = defaultdict(int)
    for _ in range(AMOUNT_OF_TIMES_TO_REPEAT):

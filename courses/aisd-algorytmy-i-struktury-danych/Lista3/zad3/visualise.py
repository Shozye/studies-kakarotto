...
def main():
    ...
    for gen_type in gen_types:
        ...
        for select_type in selects:
            for partition_size in ["3", "5", "7", "9"]:
                if partition_size == "3":
                    color = "pink" if select_type == "select" else "purple"
                if partition_size == "5":
                    color = "red" if select_type == "select" else "darkred" 
                if partition_size == "7":
                    color = "green" if select_type == "select" else "darkgreen" 
                if partition_size == "9":
                    color = "blue" if select_type == "select" else "darkblue"
                k_num=-1
                for k in ["first", "quarter", "half"]:
                    k_num+=1
                    data_path = os.path.join(DATA, select_type, gen_type, partition_size, k)
                    ...
                    LINEWIDTH = 3
                    axs[k_num][0].plot(Xs, YsC, label=f"Comparisons {select_type} partition_size {partition_size}", linewidth=LINEWIDTH, color=color)
                    
                    axs[k_num][1].plot(Xs, YsD, label=f"Displacements {select_type} partition_size {partition_size}", linewidth=LINEWIDTH, color=color)
